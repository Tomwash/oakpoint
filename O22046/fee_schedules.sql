                        ...
                        (
                            ...
                            FROM 
                            OPENROWSET(BULK 'oakpoint-data/O22046/fee_schedules.json', DATA_SOURCE = 'OakpointDataV1', SINGLE_CLOB) AS AzureBlob 
                        ); 
                        ...
                        original.updated_at = '2020-05-12T01:08:15.298Z',
                        ...
                        (
                            SELECT
                               '[' + CAST(BulkColumn AS NVARCHAR(MAX)) + ']' AS JsonData 
                            FROM 
                            OPENROWSET(BULK 'oakpoint-data/streams/O22046/fee_schedules.json', DATA_SOURCE = 'OakpointDataV1', SINGLE_CLOB) AS AzureBlob 
                        ); 
                        ...
                        WHEN MATCHED 
                            THEN UPDATE SET 
                            original.[col_href] = modified.[col_href],original.[col_procedure_code] = modified.[col_procedure_code],original.[col_fee_no] = modified.[col_fee_no],original.[col_fee_name] = modified.[col_fee_name],original.[col_fee_amount] = modified.[col_fee_amount],original.[col_practice_id] = modified.[col_practice_id],original.[col_practice] = modified.[col_practice],
                        original.updated_at = '2020-05-18T02:48:03.902Z',
                        original.office_id = 'O22046',
                        practice_name = 'Carolina Braces'
                        WHEN NOT MATCHED BY TARGET THEN
                        INSERT  
